...
unsigned char voice1[100] = "[v10][m0][t5]下雨了 回家收衣服";
...
_Bool ESP8266_SendCmd(char *cmd, char *res)
{
	...
	while(timeOut--)
	{
		...
		{
					if(strstr((const char *)esp8266_buf, "voice") != NULL)		//如果检索到关键词
		{
...
void ESP8266_Init(void)
{
	...
	ESP8266_SendCmd(ESP8266_WIFI_INFO, "");//连接WiFi，改你自己的热点
	delay_ms(1000);

	ESP8266_SendCmd(MQTTCOT, "");//mqtt账号密码，不用改
	delay_ms(1000);
			;
	ESP8266_SendCmd(MQTTID, "");//mqtt服务器，端口
	delay_ms(1500);
	ESP8266_SendCmd("AT+MQTTSUB=0,\"ZDFSUB\",1\r\n", "");//向mqtt发送数据
	delay_ms(1000);
				
	ESP8266_SendCmd(MQTTPUB, "");//发送数据
	delay_ms(1000);
